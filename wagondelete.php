...
<ol>
    <h3>Вы действительно хотите удалить данный вагон из базы?</h3>
<body>
<?php
$id = intval($_GET['G']);
$x = new mysqli('localhost', 'root', '', 'r_w');
$result = $x->query("SELECT * FROM wagons where wagon_id={$id}");
...
echo "<div class='card-group'>
  <div class='card' style='max-width: 14rem;'>
      ...
      <li class='list-group-item'>Тип вагона</li>
      <li class='list-group-item'>Год сборки</li>
      </ul>
    </div>
  <div class='card' style='max-width: 20rem;'>
      ...
      <ul class='list-group list-group-flush'>
      <li class='list-group-item'>".$r['type']."</li>
      <li class='list-group-item'>".$r['type_year']."</li>
      </ul>
    </div>
</div>";
    echo"<form class='forms' method='post'>
    <br>
    <p><input type='submit' name='pend' value='Удалить'></p>
    </form>";
    if(isset($_POST["pend"])) {
        $x->query("DELETE FROM wagons where wagon_id={$id}");
        $x->close();
        header('Location: wagonlist.php');
    }
?>
    <a href="wagonlist.php">Отменить</a>
</body>
</ol>